    ...
    <View className="gap-4">
          ...
          <Button
            ...
            onPress={async () => {
              ...
              try {
                ...
                showAlert({ title: "Error", message: "Failed to update name" });
              }
            }}
          >
            Update Name
        ...
        <CardContent className="py-4 px-4 gap-3">
          ...
          <Button
            ...
            onPress={async () => {
              ...
              try {
                await (authClient as any).changeEmail({
                  newEmail: newEmail.trim(),
                  currentPassword: emailPw,
                });
                showAlert({
                  title: "Success",
                  message: "Check your new email for verification",
                });
                setNewEmail("");
                setEmailPw("");
              } catch {
      ...
      <Card>
        <CardContent className="py-4 px-4 gap-3">
          ...
          <Text className="text-xs text-muted-foreground">
            Update your password. Use a strong password with at least 8
            characters.
          </Text>
          <Input
            label="Current Password"
      ...
      <Card>
        <CardContent className="py-4 px-4 gap-3">
          ...
          <Text className="text-xs text-muted-foreground">
            Restart the introductory tour to learn about the app features.
          </Text>
          <Button
            size="sm"
            variant="outline"
            loading={resetTours.isPending}
            onPress={async () => {
              try {
                await resetTours.mutateAsync();
                showAlert({
                  title: "Tour Reset",
                  message:
                    "The guided tour will show next time you visit the dashboard.",
                });
              } catch {
                showAlert({ title: "Error", message: "Failed to reset tour" });
              }
            }}
          >
...
const SchedulingTab = () => {
  const { showAlert } = useAlert();
  const { data: weekly, refetch: refetchWeekly } =
    trpc.availability.getWeekly.useQuery();
  const { data: travel, refetch: refetchTravel } =
    trpc.availability.getTravelSettings.useQuery();
  const { data: video, refetch: refetchVideo } =
    trpc.availability.getVideoSettings.useQuery();
  const { data: overrides, refetch: refetchOverrides } =
    trpc.availability.getOverrides.useQuery({
      startDate: new Date().toISOString(),
      endDate: new Date(Date.now() + 90 * 86400000).toISOString(),
    });
  const { data: locations, refetch: refetchLocations } =
    trpc.sessionLocation.list.useQuery();
  ...
  const [travelBuffer, setTravelBuffer] = useState(
    (travel as any)?.travelBufferMin ?? 15,
  );
  const [smartTravel, setSmartTravel] = useState(
    (travel as any)?.smartTravelEnabled ?? false,
  );
  const [offersVideo, setOffersVideo] = useState(
    (video as any)?.offersVideoSessions ?? false,
  );
  const [videoFree, setVideoFree] = useState(
    (video as any)?.videoCallsFree ?? false,
  );
  ...
  const handleAddSlot = () => {
    ...
    if (dur < 15 || dur > 180) {
      showAlert({ title: "Error", message: "Duration must be 15-180 minutes" });
      return;
    }
    const updated = [
      ...slots,
    ...
    setWeekly.mutate({ slots: updated } as any, {
      onSuccess: () => {
        refetchWeekly();
    ...
    <View className="gap-4">
                    ...
                    return (
                      ...
                      >
                        <Text className="text-sm text-foreground">
                          ...
                          {slot.sessionDurationMin ?? 60}min)
                        </Text>
                        <TouchableOpacity
                          onPress={() => handleRemoveSlot(globalIndex)}
                        >
            ...
            <View className="bg-secondary/50 rounded-lg p-3 gap-3 border border-border">
              <View className="flex-row flex-wrap gap-1">
                {DAYS.map((d) => (
                  <TouchableOpacity
                    key={d}
                    className={`px-2 py-1.5 rounded ${newSlotDay === d ? "bg-teal" : "bg-card border border-border"}`}
                    onPress={() => setNewSlotDay(d)}
                  >
                    <Text
                      className={`text-xs ${newSlotDay === d ? "text-teal-foreground" : "text-muted-foreground"}`}
                    >
                      {d.slice(0, 3)}
                    </Text>
                  </TouchableOpacity>
      ...
      <Card>
        <CardContent className="py-4 px-4 gap-3">
          ...
          <Text className="text-xs text-muted-foreground">
            Where you offer training sessions.
          </Text>
            ...
            >
              <View className="flex-1 gap-0.5">
                <View className="flex-row items-center gap-2">
                  ...
                  {loc.isDefault && (
                    <View className="bg-teal/20 rounded px-1.5 py-0.5">
                      <Text className="text-[10px] text-teal">Default</Text>
                    </View>
                  )}
                  {!loc.isActive && (
                  ...
                  <TouchableOpacity
                    onPress={() =>
                      updateLocation.mutate(
                        { id: loc.id, isDefault: true } as any,
              ...
              <AddressInput
                ...
                onSelect={(addr) => {
                  setNewLocationAddress(
                    [addr.addressLine1, addr.city, addr.postcode]
                      .filter(Boolean)
                      .join(", "),
                  );
                  setNewLocationLatLng({
        ...
        <CardContent className="py-4 px-4 gap-3">
          ...
          <View className="flex-row items-center justify-between py-2">
            <View className="flex-1 gap-0.5">
              ...
              <Text className="text-xs text-muted-foreground">
                Use Google Maps to calculate actual travel time.
              </Text>
            </View>
            <Switch
              value={smartTravel}
              onValueChange={setSmartTravel}
          ...
          <Button
              ...
              updateTravel.mutate(
                {
                  ...
                  smartTravelEnabled: smartTravel,
                } as any,
                {
                  onSuccess: () => {
                    refetchTravel();
            ...
            loading={updateTravel.isPending}
          >
            Save
          </Button>
        </CardContent>
          ...
          <View className="flex-row items-center justify-between py-2">
            ...
            <Switch
              ...
              onValueChange={(v) => {
                ...
                updateVideo.mutate(
                  {
                    offersVideoSessions: v,
                    ...(v ? {} : { videoCallsFree: false }),
                  } as any,
            ...
            <View className="flex-row items-center justify-between py-2">
              ...
              <Switch
                ...
                onValueChange={(v) => {
                  setVideoFree(v);
                  updateVideo.mutate(
                    { offersVideoSessions: true, videoCallsFree: v } as any,
                    { onSuccess: () => refetchVideo() },
                  );
                }}
                trackColor={{ false: colors.muted, true: colors.teal }}
        ...
        <CardContent className="py-4 px-4 gap-3">
          <View className="flex-row items-center justify-between">
            ...
            <TouchableOpacity onPress={() => setShowBlockDate(!showBlockDate)}>
              <Text className="text-sm text-teal">
                {showBlockDate ? "Cancel" : "Block Date"}
              </Text>
            </TouchableOpacity>
          </View>
            ...
            <View className="bg-secondary/50 rounded-lg p-3 gap-3 border border-border">
              ...
              <View className="flex-row gap-2">
                ...
                <Button
                  ...
                  variant="outline"
                  className="flex-1"
                  onPress={() => setShowBlockDate(false)}
                >
                  Cancel
                </Button>
              </View>
          ...
          {isConnected ? (
            <View className="gap-2">
              <Button
                ...
                onPress={async () => {
                  try {
                    ...
                    if ((result as any).url)
                      await WebBrowser.openBrowserAsync((result as any).url);
                  } catch {
                    showAlert({
                      title: "Error",
                ...
                loading={getDashboard.isPending}
              >
            ...
            <Button
              ...
              onPress={async () => {
                try {
                  ...
                  showAlert({
                    title: "Error",
                    message: "Failed to start Stripe setup",
                  });
                }
              }}
          ...
          <CardContent className="py-4 px-4 gap-3">
            ...
            <View className="flex-row items-center justify-between py-2">
              <View className="flex-1 gap-0.5">
                ...
                <Text className="text-xs text-muted-foreground">
                  Enable session payment collection from clients.
                </Text>
              </View>
              <Switch
                value={s?.paymentsEnabled ?? false}
                onValueChange={(v) => {
            ...
            <View className="flex-row items-center justify-between py-2">
              <View className="flex-1 gap-0.5">
                ...
                <Text className="text-xs text-muted-foreground">
                  New clients get their first session at no charge.
                </Text>
              </View>
              <Switch
                value={s?.firstSessionFree ?? false}
                onValueChange={(v) => {
                  updateSettings.mutate(
                    { firstSessionFree: v },
                    { onSuccess: () => refetch() },
      ...
      {isConnected && (
        <Card>
          <CardContent className="py-4 px-4 gap-3">
            ...
            <Text className="text-xs text-muted-foreground">
              Set the price per session (£1 - £1,000).
            </Text>
            <Input
            ...
            <Button
              ...
              onPress={() => {
                ...
                if (isNaN(amount) || amount < 100 || amount > 100000) {
                  showAlert({
                    title: "Error",
                    message: "Price must be between £1 and £1,000",
                  });
                  return;
                }
                updatePrice.mutate(
      ...
      {isConnected && s?.cancellationPolicy && (
        <Card>
          <CardContent className="py-4 px-4 gap-3">
            ...
            <Text className="text-xs text-muted-foreground">
              Full refund: {s.cancellationPolicy.fullRefundHours}h before ·
              Partial refund: {s.cancellationPolicy.partialRefundPercent}% if
              cancelled {s.cancellationPolicy.partialRefundHours}h before
            </Text>
            <Text className="text-xs text-muted-foreground">
              Edit cancellation policy in the web app for full control.
            </Text>
          </CardContent>
        </Card>
      )}
...
> = {
  ...
  FITBIT: {
    ...
    color: "#00B0B9",
    dataTypes: ["Steps", "Sleep", "Heart Rate", "Water", "Weight"],
      ...
      <Card>
        <CardContent className="py-4 px-4 gap-1">
          ...
          <Text className="text-xs text-muted-foreground mb-2">
            Connect your fitness apps to automatically sync activities, steps,
            sleep and more.
          </Text>
        </CardContent>
      </Card>
...
const DangerZoneTab = () => {
  ...
  return (
    <Card>
      <CardContent className="py-4 px-4 gap-3">
        <Text
          className="text-sm font-medium text-destructive uppercase"
          style={{ letterSpacing: 1 }}
        >
          Danger Zone
        </Text>
...
const SettingsScreen = () => {
  ...
  const tabs: { key: SettingsTab; label: string }[] = [
    ...
    ...(isTrainer
      ? [{ key: "scheduling" as SettingsTab, label: "Scheduling" }]
      : []),
    ...(isTrainer
      ? [{ key: "payments" as SettingsTab, label: "Payments" }]
      : []),
    ...(!isTrainer
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      ...
      >
        ...
        {tab === "scheduling" &&
          (hasFeatureAccess(currentTier, "booking") ? (
            <SchedulingTab />
          ) : (
            <Card>
              <CardContent className="py-6 px-4 items-center gap-2">
                <Text className="text-sm text-muted-foreground text-center">
                  Upgrade to Pro to access scheduling settings.
                </Text>
                <Button
                  size="sm"
        ...
        {tab === "payments" &&
          (hasFeatureAccess(currentTier, "sessionPayments") ? (
            <PaymentsTab />
          ) : (
            <Card>
              <CardContent className="py-6 px-4 items-center gap-2">
                <Text className="text-sm text-muted-foreground text-center">
                  Upgrade to Elite to access payment settings.
                </Text>
                <Button
                  size="sm"
                  onPress={() => router.push("/dashboard/subscription")}
