...
const getServiceLabel = (value: string) =>
  TRAINER_SERVICES.find((s) => s.value === value)?.label ??
  value.replace(/-/g, " ");
const getQualLabel = (value: string) =>
  TRAINER_QUALIFICATIONS.find((q) => q.value === value)?.label ??
  value.replace(/-/g, " ");
const getPostcodeArea = (postcode?: string | null) =>
  postcode?.split(" ")[0] ?? postcode;
...
const TRAVEL_INFO: Record<string, { icon: any; label: string; desc: string }> =
  {
    CLIENT_TRAVELS: {
      ...
      desc: "Clients travel to the trainer",
    },
    TRAINER_TRAVELS: {
      icon: Car,
      label: "Mobile Trainer",
      desc: "Trainer travels to clients",
    },
    BOTH: {
      icon: ArrowLeftRight,
      label: "Flexible Location",
      desc: "Both options available",
    },
  };
...
const TrainerProfileScreen = () => {
  ...
  const reviewsQuery = trpc.review.getByTrainer.useInfiniteQuery(
    ...
    {
      enabled: !!(trainer as any)?.id,
      getNextPageParam: (last: any) => last.nextCursor,
    },
  );
  const reviews =
    reviewsQuery.data?.pages.flatMap((p: any) => p.reviews ?? []) ?? [];

  // Products & Blog (Elite)
  const hasWebsite =
    (trainer as any)?.website?.subdomain &&
    (trainer as any)?.website?.status === "PUBLISHED";
  ...
  const handleConnect = () => {
    ...
    submitConnect.mutate({ trainerId: t?.id, message: connectMsg } as any, {
      onSuccess: () => {
        ...
        setConnectMsg("");
        showAlert({ title: "Sent", message: "Connection request sent!" });
        pendingCheck.refetch();
      },
      onError: () =>
        showAlert({ title: "Error", message: "Failed to send request" }),
    });
  };
    ...
    submitCallback.mutate(
      ...
      } as any,
      {
        onSuccess: () => {
          ...
          setCallbackPhone("");
          setCallbackMsg("");
          showAlert({ title: "Sent", message: "Callback request sent!" });
          pendingCheck.refetch();
        },
        onError: () =>
          showAlert({ title: "Error", message: "Failed to send request" }),
    ...
    <SafeAreaView className="flex-1 bg-background" edges={["bottom"]}>
      <ScrollView className="flex-1" contentContainerClassName="pb-24">
        ...
        <View className="relative">
          {t.coverImageUrl ? (
            <Image
              source={{ uri: t.coverImageUrl }}
              className="w-full h-48"
              resizeMode="cover"
            />
          ) : (
            <GradientBackground style={{ height: 192 }}>
              <View />
            </GradientBackground>
          )}
          {/* Back button overlay */}
        ...
        <View className="mt-14 px-4 gap-1">
          ...
          {(t.city || t.postcode) && (
            <View className="flex-row items-center gap-1">
              ...
              <Text className="text-sm text-muted-foreground">
                {t.city}
                {t.postcode ? `, ${getPostcodeArea(t.postcode)} area` : ""}
              </Text>
            </View>
          )}
          ...
          <View className="flex-row items-center gap-1 mt-1">
            {t.acceptingClients ? (
              ...
              <>
                <UserX size={14} color={colors.mutedForeground} />
                <Text className="text-sm text-muted-foreground">
        ...
        {user && role === "TRAINEE" && t.userId !== user.id && (
          <View className="flex-row items-center gap-3 px-4 mt-3">
            ...
            >
              <View className="flex-row items-center gap-1">
                {isFollowing ? (
                  <UserMinus size={14} color={colors.foreground} />
                ) : (
                  <UserPlus size={14} color="#fff" />
                )}
                <Text
                  className={`text-sm font-semibold ${isFollowing ? "text-foreground" : "text-white"}`}
                >
          ...
          {t.videoIntroUrl && (
            <Card>
              <CardContent className="py-4 px-4 gap-2">
                ...
                <TouchableOpacity
                  onPress={() => Linking.openURL(t.videoIntroUrl)}
                >
                ...
                <View className="flex-row flex-wrap gap-2">
                  {t.galleryImages.map((img: any) => (
                    <TouchableOpacity
                      key={img.id}
                      onPress={() =>
                        setGalleryImage(typeof img === "string" ? img : img.url)
                      }
            ...
            <Card>
              <CardContent className="py-4 px-4 gap-2">
                ...
                {GOOGLE_API_KEY && (
                  <Image
                    source={{
                      uri: `https://maps.googleapis.com/maps/api/staticmap?center=${t.latitude},${t.longitude}&zoom=12&size=400x200&scale=2&key=${GOOGLE_API_KEY}`,
                    }}
                ...
                {t.postcode && (
                  <Text className="text-xs text-muted-foreground">
                    {getPostcodeArea(t.postcode)} area
                  </Text>
                )}
                {travelInfo && (
            ...
            <Card>
              <CardContent className="py-4 px-4 gap-3">
                    ...
                    <View className="flex-row gap-0.5">
                      {[1, 2, 3, 4, 5].map((i) => (
                        <Star
                          ...
                          color={colors.teal}
                          fill={
                            i <= Math.round(t.ratingAverage ?? 0)
                              ? colors.teal
                              : "transparent"
                          }
                        />
                ...
                {reviews.slice(0, 5).map((review: any) => (
                  <View
                    key={review.id}
                    className="gap-1 py-2 border-b border-border"
                  >
                    <View className="flex-row items-center gap-2">
                      <Text className="text-sm font-medium text-foreground">
                      ...
                      <View className="flex-row gap-0.5">
                        {[1, 2, 3, 4, 5].map((i) => (
                          <Star
                            ...
                            color={colors.teal}
                            fill={
                              i <= review.rating ? colors.teal : "transparent"
                            }
                          />
                        ))}
                      </View>
                ...
                {reviewsQuery.hasNextPage && (
                  ...
                  >
                    <Text className="text-sm text-teal text-center py-2">
                      Load more reviews
                    </Text>
                  </TouchableOpacity>
                )}
            ...
            <Card>
              <CardContent className="py-4 px-4 gap-2">
                <View className="flex-row items-center justify-between">
                  ...
                  <TouchableOpacity
                    onPress={() => handle && router.push(`/shop/${handle}`)}
                  >
                ...
                {products.map((p: any) => (
                  ...
                  >
                    ...
                    <Text className="text-sm font-medium text-teal">
                      £{((p.priceInCents ?? 0) / 100).toFixed(2)}
                    </Text>
                  </View>
                ))}
                <Button
          ...
          {isConnected ? (
            <View className="flex-row gap-2">
              <Button
                ...
                className="flex-1"
                onPress={() =>
                  connectionId && router.push(`/messages/${connectionId}`)
                }
              >
                <View className="flex-row items-center gap-1">
              ...
              <Button
                className="flex-1"
                onPress={() => t?.id && router.push(`/bookings/book/${t.id}`)}
              >
                <View className="flex-row items-center gap-1">
      ...
      <Modal
        ...
        transparent
        animationType="fade"
        onRequestClose={() => setGalleryImage(null)}
      >
      ...
      >
        <SafeAreaView className="flex-1 bg-background">
          ...
          <View className="px-4 py-4 gap-4">
            ...
            <Button onPress={handleConnect} loading={submitConnect.isPending}>
              Send Request
            </Button>
          </View>
        </SafeAreaView>
      ...
      <Modal
        ...
        animationType="slide"
        presentationStyle="pageSheet"
        onRequestClose={() => setShowCallback(false)}
      >
        <SafeAreaView className="flex-1 bg-background">
          <View className="flex-row items-center justify-between px-4 py-4 border-b border-border">
            <Text className="text-base font-semibold text-foreground">
            ...
            <TouchableOpacity onPress={() => setShowCallback(false)}>
              <X size={24} color={colors.foreground} />
            </TouchableOpacity>
          </View>
          <View className="px-4 py-4 gap-4">
            ...
            <Button onPress={handleCallback} loading={submitCallback.isPending}>
              Request Callback
            </Button>
          </View>
        </SafeAreaView>
