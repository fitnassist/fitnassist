...
import { formatPricePence } from "@fitnassist/schemas/src/constants/subscription.constants";
...
const BookSessionScreen = () => {
  ...
  const [clientAddress, setClientAddress] = useState<AddressResult | null>(
    null,
  );
  const [notes, setNotes] = useState("");

  const { data: trainerInfo } = trpc.trainer.getById.useQuery(
    { id: trainerId! },
    { enabled: !!trainerId },
  );
  const { data: myRoster } = trpc.clientRoster.myTrainers.useQuery(undefined, {
    enabled: !!trainerId,
  });
  const clientRosterId =
    (myRoster as any[])?.find(
      (r: any) => r.trainer?.id === trainerId || r.trainerId === trainerId,
    )?.id ?? "";
  ...
  const { data: trainerLocations } =
    trpc.sessionLocation.listByTrainer.useQuery(
      { trainerId: trainerId! },
      { enabled: !!trainerId && step === "location" },
    );

  const { data: paymentRequirement } =
    trpc.payment.getPaymentRequirement.useQuery(
      { trainerId: trainerId!, clientRosterId, sessionType },
      { enabled: !!trainerId && !!clientRosterId },
    );

  const createBooking = trpc.booking.create.useMutation();
  const createCheckoutSession =
    trpc.payment.createBookingCheckoutSession.useMutation();
  ...
  const goNext = () => {
    if (step === "date") setStep("time");
      ...
      setStep(offersVideo ? "session-type" : "location");
    else if (step === "session-type")
      setStep(sessionType === "VIDEO_CALL" ? "confirm" : "location");
  ...
  const goBack = () => {
    if (step === "time") setStep("date");
      ...
      setStep(offersVideo ? "session-type" : "time");
    else if (step === "confirm")
      setStep(sessionType === "VIDEO_CALL" ? "session-type" : "location");
    else router.back();
  };
  ...
  const handleSubmit = async () => {
    if (!trainerId || !selectedDate || !selectedSlot || !clientRosterId) {
      showAlert({
        title: "Error",
        message: !clientRosterId
          ? "You must be connected to this trainer to book."
    ...
    try {
      ...
      if (paymentRequirement?.paymentRequired && bookingId) {
        ...
        try {
          ...
          showAlert({
            ...
            message:
              "Your booking was created but payment could not be started. You can pay later from your booking.",
            actions: [
              {
                label: "OK",
                onPress: () => router.replace(`/bookings/${bookingId}` as any),
              },
            ],
          });
        }
      ...
      >
              ...
              {[
                {
                  ...
                  label: "In Person",
                  icon: MapPin,
                  desc: "Meet at a location",
                },
                {
                  type: "VIDEO_CALL" as const,
                  label: "Video Call",
                  icon: Video,
                  desc: "Train from anywhere",
                },
                ...
                >
                  ...
                  />
                  <Text
                    className={`text-sm font-medium ${sessionType === type ? "text-primary" : "text-foreground"}`}
                  >
                    {label}
                  </Text>
        ...
        {step === "location" && (
          <>
            ...
            {(trainerLocations as any[])?.length > 0 && (
              <>
                ...
                {(trainerLocations as any[]).map((loc: any) => (
                  ...
                  >
                    <Text
                      className={`text-sm font-medium ${selectedLocationId === loc.id ? "text-teal" : "text-foreground"}`}
                    >
                      {loc.name}
                    </Text>
                    {loc.address && (
            ...
            {!trainerLocations?.length && (
              <Text className="text-xs text-muted-foreground">
                Enter your address for this session
              </Text>
            )}
            <AddressInput
        ...
        {step === "confirm" && selectedSlot && (
          <>
              ...
              <CardContent className="py-4 px-4 gap-3">
                {[
                  ...
                  <View key={label}>
                    ...
                    {i < arr.length - 1 && (
                      <View className="border-b border-border mt-3" />
                    )}
                  </View>
                ))}
              </CardContent>
            ...
            {/* Payment info */}
            {paymentRequirement?.paymentRequired && (
              <Card>
                <CardContent className="py-3 px-4 flex-row justify-between items-center">
                  <Text className="text-sm text-muted-foreground">
                    Session Price
                  </Text>
                  <Text className="text-base font-semibold text-foreground">
                    {formatPricePence(paymentRequirement.amount)} per session
                  </Text>
                </CardContent>
              </Card>
            )}
            {paymentRequirement &&
              !paymentRequirement.paymentRequired &&
              paymentRequirement.reason === "first_session_free" && (
                <View className="items-center">
                  <Badge variant="default">First Session Free</Badge>
                </View>
              )}
            ...
            <Button
              ...
              loading={createBooking.isPending || isProcessingPayment}
            >
              {isProcessingPayment
                ? "Processing payment..."
                : paymentRequirement?.paymentRequired
