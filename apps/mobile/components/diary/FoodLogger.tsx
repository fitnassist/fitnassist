...
export const FoodLogger = ({ visible, onClose, date }: FoodLoggerProps) => {
  ...
  const { data: searchResults, isLoading: searching } =
    trpc.diary.searchFood.useQuery({ query }, { enabled: query.length >= 2 });
  ...
  const handleSubmit = () => {
    ...
    logFood.mutate(
      ...
      },
      {
        ...
        onError: () =>
          showAlert({ title: "Error", message: "Failed to log food" }),
      },
    );
  };
    ...
    <LoggerModal visible={visible} onClose={onClose} title="Log Food">
      ...
      <View className="flex-row gap-1">
        {MEAL_TYPES.map((type) => (
          ...
          >
            <Text
              className={`text-xs font-medium ${mealType === type ? "text-white" : "text-muted-foreground"}`}
            >
      ...
      {mode === "search" ? (
        <>
          ...
          {searchResults && (searchResults as any)?.products?.length > 0 && (
            <ScrollView
              className="max-h-64 bg-card border border-border rounded-lg"
                ...
                .map((food: any, i: number) => (
                  ...
                  >
                    ...
                    <Text className="text-xs text-muted-foreground">
                      {food.calories} kcal · {food.serving_qty}
                      {food.serving_unit}
                    </Text>
                  </TouchableOpacity>
                ))}
