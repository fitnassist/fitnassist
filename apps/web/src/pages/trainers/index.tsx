...
export const TrainersPage = () => {
  ...
  const [acceptingClients, setAcceptingClients] = useState(searchParams.get('accepting') === 'true');

  const hasLocation = !!(location.latitude && location.longitude);
  const defaultSort: SortBy = hasLocation ? 'distance' : 'newest';
  const [sortBy, setSortBy] = useState<SortBy>(
    (searchParams.get('sort') as SortBy) || defaultSort
  );
  ...
  const { data, isLoading, error } = trpc.trainer.search.useQuery({
    ...
    travelOption: travelOption ? travelOption as 'CLIENT_TRAVELS' | 'TRAINER_TRAVELS' | 'BOTH' : undefined,
  ...
  const handleServicesChange = useCallback((newServices: string[]) => {
    setServices(newServices);
    updateParam('services', newServices.length > 0 ? newServices.join(',') : undefined);
  }, [updateParam]);

  const handleQualificationsChange = useCallback((newQualifications: string[]) => {
    setQualifications(newQualifications);
    updateParam('qualifications', newQualifications.length > 0 ? newQualifications.join(',') : undefined);
  }, [updateParam]);
  ...
  const handleTravelOptionChange = useCallback((option: string) => {
    setTravelOption(option);
    updateParam('travelOption', option || undefined);
  }, [updateParam]);

  const handleMinRateChange = useCallback((rate: number | undefined) => {
    setMinRate(rate);
    updateParam('minRate', rate !== undefined ? rate.toString() : undefined);
  }, [updateParam]);

  const handleMaxRateChange = useCallback((rate: number | undefined) => {
    setMaxRate(rate);
    updateParam('maxRate', rate !== undefined ? rate.toString() : undefined);
  }, [updateParam]);

  const handleAcceptingClientsChange = useCallback((accepting: boolean) => {
