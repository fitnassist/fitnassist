...
import {
  ...
  today,
} from '@/pages/dashboard/diary/diary.utils';
...
export const ClientProgress = ({ clientRosterId, traineeUserId }: ClientProgressProps) => {
  ...
  const activityData = (clientEntries ?? [])
    ...
    .map((e) => ({
      date: e.date as unknown as string,
      activityType: (e as unknown as { activityEntry: { activityType: string } }).activityEntry
        .activityType,
      distanceKm: (e as unknown as { activityEntry: { distanceKm: number | null } }).activityEntry
        .distanceKm,
      durationSeconds: (e as unknown as { activityEntry: { durationSeconds: number } })
        .activityEntry.durationSeconds,
    }));
...
const DateGroup = ({
  ...
  const allComments = entries
    .flatMap((e) => (e.comments ?? []).map((c) => ({ ...c, entryId: e.id })))
...
const getEntryContent = (
  ...
  switch (entry.type) {
    case 'WEIGHT':
      return entry.weightEntry
        ? { label: 'Weight', value: formatWeight(entry.weightEntry.weightKg, unitPreference) }
        : null;
    case 'WATER':
      return entry.waterEntry
        ? { label: 'Water', value: formatWater(entry.waterEntry.totalMl, unitPreference) }
        : null;
    case 'MOOD':
      return entry.moodEntry
        ? {
            label: 'Mood',
            value: `${getMoodEmoji(entry.moodEntry.level)} ${entry.moodEntry.level}${entry.moodEntry.notes ? ` — ${entry.moodEntry.notes}` : ''}`,
          }
        : null;
    case 'SLEEP':
      return entry.sleepEntry
        ? {
            label: 'Sleep',
            value: `${entry.sleepEntry.hoursSlept}h — ${getSleepQualityLabel(entry.sleepEntry.quality)}`,
          }
        : null;
    case 'FOOD': {
      ...
      return {
        label: 'Food',
        value: `${entry.foodEntries.length} items — ${cals.toLocaleString()} kcal`,
      };
    }
    case 'WORKOUT_LOG': {
      if (!entry.workoutLogEntry) return null;
      const name =
        entry.workoutLogEntry.workoutPlanName ||
        entry.workoutLogEntry.workoutPlan?.name ||
        'Freeform';
      return {
        label: 'Workout',
        value: `${name} — ${formatDuration(entry.workoutLogEntry.durationMinutes)}${entry.workoutLogEntry.caloriesBurned ? ` (${entry.workoutLogEntry.caloriesBurned} kcal)` : ''}`,
      };
    }
    case 'STEPS':
      return entry.stepsEntry
        ? { label: 'Steps', value: `${entry.stepsEntry.totalSteps.toLocaleString()} steps` }
        : null;
    case 'MEASUREMENT': {
      ...
      const parts = Object.entries(entry.measurementEntry)
        ...
        .map(
          ([k, v]) => `${k.replace('Cm', '')}: ${formatMeasurement(v as number, unitPreference)}`,
        );
    ...
    case 'ACTIVITY': {
      ...
      if (act.avgPaceSecPerKm != null && act.avgPaceSecPerKm > 0)
        parts.push(formatPace(act.avgPaceSecPerKm));
    ...
    case 'PROGRESS_PHOTO':
      return entry.progressPhotos && entry.progressPhotos.length > 0
        ? {
            label: 'Photos',
            value: `${entry.progressPhotos.length} photo${entry.progressPhotos.length > 1 ? 's' : ''}`,
          }
        : null;
    default:
      return null;
...
const EntryRow = ({
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2.5">
      <div
        className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-muted ${iconColor}`}
      >
      ...
      {entry.type === 'PROGRESS_PHOTO' &&
        entry.progressPhotos &&
