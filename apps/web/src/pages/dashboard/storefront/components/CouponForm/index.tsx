      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-lg">
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Customers enter this at checkout. Auto-uppercase, no spaces.
          </p>
        </div>
        ...
        {discountType === 'PERCENT' ? (
          <div className="space-y-2">
            ...
            />
            {errors.percentOff && (
              <p className="text-sm text-destructive">{errors.percentOff.message}</p>
            )}
          </div>
        ) : (
          <div className="space-y-2">
            ...
            <Input
              id="amountOffPence"
              type="number"
              {...register('amountOffPence', { valueAsNumber: true })}
              placeholder="e.g. 500 for £5.00"
              min={1}
            />
            {errors.amountOffPence && (
              <p className="text-sm text-destructive">{errors.amountOffPence.message}</p>
            )}
          </div>
        )}
        ...
        <div className="space-y-2">
          ...
          <Input
            id="minOrderPence"
            type="number"
            {...register('minOrderPence', { valueAsNumber: true })}
            placeholder="e.g. 2000 for £20 minimum"
          />
        </div>
