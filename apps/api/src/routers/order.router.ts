...
export const orderRouter = router({
  ...
  myOrders: protectedProcedure
    .input(z.object({ cursor: z.string().optional(), limit: z.number().int().min(1).max(50).optional() }).optional())
    .query(async ({ ctx, input }) => {
      return productPaymentService.getBuyerOrders(ctx.user.id, input?.cursor, input?.limit);
    }),

  getDownloadUrl: protectedProcedure
    .input(z.object({ orderId: z.string(), productId: z.string() }))
    .query(async ({ ctx, input }) => {
      return productPaymentService.getDownloadUrl(input.orderId, input.productId, ctx.user.id);
    }),
  ...
  trainerOrders: trainerProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return productPaymentService.getTrainerOrders(profile.id, input?.cursor, input?.limit);
    }),
  ...
  updateStatus: trainerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const profile = await requireTrainerProfile(ctx.user.id);
      return productPaymentService.updateOrderStatus(profile.id, input.orderId, input.status);
    }),
  ...
  refund: trainerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const profile = await requireTrainerProfile(ctx.user.id);
      return productPaymentService.refundOrder(input.orderId, profile.id, input.reason);
    }),
});
