...
const verifyReadAccess = async (
  ...
  const roster = await clientRosterRepository.findByTrainerAndTraineeUserId(
    trainer.id,
    targetUserId,
  );
  if (!roster) {
...
const getTrainerRosters = async (traineeUserId: string) => {
  const rosters =
    await clientRosterRepository.findByTraineeUserId(traineeUserId);
 ...
 */
const broadcastDiaryEntry = async (
  userId: string,
  entryType: string,
  date: string,
) => {
 ...
 */
const broadcastDiaryComment = async (
  commenterId: string,
  diaryEntryId: string,
  entryOwnerId: string,
  date: string,
...
export const diaryService = {
  async logWeight(
    ...
    const result = await diaryRepository.upsertWeight(
      userId,
      parseDate(data.date),
      data.weightKg,
      data.source,
    ...
    goalService
      .checkAutoProgress(userId, "WEIGHT", data.weightKg)
      .catch(() => {});
    personalBestService
      .checkWeightPB(userId, data.weightKg, parseDate(data.date))
      .catch(() => {});
  ...
  async logWater(
    ...
    const userId = await resolveUserId(callerId, data.clientRosterId);
    const result = await diaryRepository.upsertWater(
      userId,
      parseDate(data.date),
      data.totalMl,
      data.source,
  ...
  async logMeasurements(
    ...
    const result = await diaryRepository.upsertMeasurements(
      userId,
      parseDate(date),
      measurements,
    );
  ...
  async logMood(
    ...
    const userId = await resolveUserId(callerId, data.clientRosterId);
    const result = await diaryRepository.upsertMood(
      userId,
      parseDate(data.date),
      data.level,
      data.notes,
  ...
  async logSleep(
    ...
    const userId = await resolveUserId(callerId, data.clientRosterId);
    const result = await diaryRepository.upsertSleep(
      userId,
      parseDate(data.date),
      data.hoursSlept,
      data.quality,
  ...
  async logWorkout(
    ...
    const result = await diaryRepository.createWorkoutLog(
      userId,
      parseDate(date),
      workoutData,
    );
    ...
    personalBestService
      .checkWorkoutPB(userId, data.durationMinutes, parseDate(date))
      .catch(() => {});
  ...
  async logSteps(
    ...
    const userId = await resolveUserId(callerId, data.clientRosterId);
    const result = await diaryRepository.upsertSteps(
      userId,
      parseDate(data.date),
      data.totalSteps,
      data.source,
    ...
    personalBestService
      .checkStepsPB(userId, data.totalSteps, parseDate(data.date))
      .catch(() => {});
  ...
  async logActivity(
    ...
    const avgPaceSecPerKm =
      activityData.distanceKm && activityData.distanceKm > 0
        ? Math.round(activityData.durationSeconds / activityData.distanceKm)
        : undefined;

    const result = await diaryRepository.createActivity(
      userId,
      parseDate(date),
      {
        ...activityData,
  ...
  async logProgressPhotos(
    ...
    const userId = await resolveUserId(callerId, data.clientRosterId);
    const diaryEntry = await diaryRepository.getOrCreateProgressPhotoDiaryEntry(
      userId,
      parseDate(data.date),
    );
  ...
  async addComment(callerId: string, diaryEntryId: string, content: string) {
    ...
    const comment = await diaryRepository.createComment(
      diaryEntryId,
      callerId,
      content,
    );
  ...
  async getRecentClientActivity(callerId: string, limit: number) {
    ...
    const entries = await diaryRepository.findRecentClientActivity(
      clientUserIds,
      limit,
    );
  ...
  async getEntries(callerId: string, data: { userId?: string; date: string }) {
    const targetUserId = data.userId || callerId;
    await verifyReadAccess(callerId, targetUserId);
    return diaryRepository.findEntriesByDate(
      targetUserId,
      parseDate(data.date),
    );
  },
  ...
  async lookupBarcode(barcode: string) {
    return foodSearch.lookupBarcode(barcode);
  },
  ...
  async logFood(
    ...
    const userId = await resolveUserId(callerId, data.clientRosterId);
    const diaryEntry = await diaryRepository.getOrCreateFoodDiaryEntry(
      userId,
      parseDate(data.date),
    );
  ...
  async updateFoodEntry(
    ...
    if (entry.diaryEntry.userId !== callerId) {
      throw new TRPCError({
        code: "FORBIDDEN",
        message: "You can only edit your own food entries",
      });
    }
  ...
  async deleteFoodEntry(callerId: string, id: string) {
    ...
    if (entry.diaryEntry.userId !== callerId) {
      throw new TRPCError({
        code: "FORBIDDEN",
        message: "You can only delete your own food entries",
      });
    }
  ...
  async getDailyNutrition(
    ...
    const targetUserId = data.userId || callerId;
    await verifyReadAccess(callerId, targetUserId);
    const nutrition = await diaryRepository.getDailyNutrition(
      targetUserId,
      parseDate(data.date),
    );
    const caloriesBurned = await diaryRepository.getDailyCaloriesBurned(
      targetUserId,
      parseDate(data.date),
    );
